...
function writeToRawLogs(ss, employeeName, dateString, timeString) {
  ...
  if (lastRow > 1) {
    ...
    for (var i = values.length - 1; i >= 0; i--) {
      if (values[i][0] === employeeName && values[i][1] === dateString) {
        matchRowIndex = i + 2;
        existingTimes = values[i][2];
        break;
      }
    }
  }
  ...
  if (matchRowIndex !== -1) {
    ...
    sheet.getRange(matchRowIndex, 3).setValue(newTimes);
  } else {
    sheet.appendRow([employeeName, dateString, timeString]);
  }
}
...
function externalHikvisionBulkSync(data, ss) {
  ...
  try {
    ...
    for (var idx = 0; idx < events.length; idx++) {
      ...
      if (result.status === "success") {
        importedCount++;
        bulkDetails.push(employeeName + " (" + (result.type === "in" ? "Vào" : "Ra") + " lúc " + gioString + " ngày " + dateStringObj + ")");
      }
    }

    // 3. Ghi trả dữ liệu Logs bằng 1 cuộc gọi setValues
    if (existingLogs.length > 0) {
      logsSheet.getRange(2, 1, existingLogs.length, 3).setValues(existingLogs);
    }

    // 4. Ghi trả dữ liệu Bang_Cham_Cong_Log bằng 1 cuộc gọi setValues
    if (existingRecords.length > 0) {
      sheet.getRange(2, 1, existingRecords.length, 5).setValues(existingRecords);
    }

    // 5. Gửi thông báo tổng hợp duy nhất nếu có dữ liệu đồng bộ thành công
    if (importedCount > 0) {
      var summaryMsg = "🔄 <b>[KING'S GRILL - ĐỒNG BỘ BÙ THÀNH CÔNG]</b>\n" +
                       "Đã đồng bộ bổ sung thành công <b>" + importedCount + "</b> lượt chấm công:\n" +
                       bulkDetails.slice(0, 15).map(function(d) { return "- " + d; }).join("\n");
      if (bulkDetails.length > 15) {
        summaryMsg += "\n... và " + (bulkDetails.length - 15) + " lượt khác.";
      }
      sendTelegram(summaryMsg, ss);
      
      // Gửi Markdown summary đến Google Chat
      var gcSummaryMsg = "🔄 *[KING'S GRILL - ĐỒNG BỘ BÙ THÀNH CÔNG]*\n" +
                         "Đã đồng bộ bổ sung thành công *" + importedCount + "* lượt chấm công:\n" +
                         bulkDetails.slice(0, 15).map(function(d) { return "- " + d; }).join("\n");
      if (bulkDetails.length > 15) {
        gcSummaryMsg += "\n... và " + (bulkDetails.length - 15) + " lượt khác.";
      }
      sendGoogleChat(gcSummaryMsg, ss);
    }
